...
import { paymentFormSchema } from "~/utils/paymentSchema.ts";
...
export default function PaymentForm({item}: Props) {
  ...
  return (
<div className="container flex items-center justify-center w-full h-screen px-16 m-auto ">
  <div className="flex flex-wrap items-center justify-between  shadow-[-10px_-10px_30px_4px_rgba(0,0,0,0.1),_10px_10px_30px_4px_rgba(135,43,151,0.15)] bg-gray-50 rounded-xl">
    <div className="w-full p-4 md:w-1/2">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <button type="submit" className="w-full ml-5 bg-secondary text-white py-2 rounded-lg cursor-pointer hover:bg-primary transition-all text-[15px]">Proccess to Pay</button>
      </form>
    </div>
    ...
    {item && (
      <div className="w-full p-4 md:w-1/2">
        <div className="px-2 py-0">
          ...
          <p className="font-light text-gray-600 sm:text-sm dark:text-slate-400">{item.description}</p>
          <div className="my-2">
            <div className="flex items-center justify-start mb-1 space-x-1 text-start">
              ...
              <span className="leading-6 text-gray-600 lowercase text-end dark:text-slate-400">/{item.time}</span>
            </div>
          </div>
          {item.points && (
            <ul className="my-2 space-y-1 text-left">
              {item.points.map((point, index) =>
                point && (
                  <p key={index} className="mb-1.5 flex items-start space-x-3 leading-7">
